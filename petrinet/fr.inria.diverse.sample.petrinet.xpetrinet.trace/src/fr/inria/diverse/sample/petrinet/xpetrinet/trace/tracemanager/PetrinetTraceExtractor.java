...
public class PetrinetTraceExtractor implements ITraceExtractor {

	private petrinetTrace.SpecificTrace traceRoot;

	final private List<List<? extends petrinetTrace.States.Value>> valueTraces = Collections
	...
	private final List<petrinetTrace.States.Value> observedValues = new ArrayList<>();

	public PetrinetTraceExtractor() {
	...
	private petrinetTrace.States.Value getValueAt(int traceIndex, int stateIndex) {
	...
	public ValueWrapper getValueWrapper(int traceIndex, int stateIndex) {
		petrinetTrace.States.Value value = getValueAt(traceIndex, stateIndex);
		if (value == null) {
			return null;
		}
		List<petrinetTrace.States.State> states = value.getStatesNoOpposite();
	...
	private List<Integer> computeStateComparisonList(List<petrinetTrace.States.Value> values) {
		final List<Integer> valueIndexes = new ArrayList<>();
		for (int i = 0; i < values.size(); i++) {
			final petrinetTrace.States.Value value = values.get(i);
			int idx = -1;
	...
	private void updateEquivalenceClasses(petrinetTrace.States.State state) {
		final List<petrinetTrace.States.Value> values = getAllStateValues(state, true);
	...
	private void updateEquivalenceClasses(List<petrinetTrace.States.State> states) {
	...
	public boolean compareStates(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final petrinetTrace.States.State state1;
		final petrinetTrace.States.State state2;

		if (eObject1 instanceof petrinetTrace.States.State) {
			state1 = (petrinetTrace.States.State) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof petrinetTrace.States.State) {
			state2 = (petrinetTrace.States.State) eObject2;
		} else {
			return false;
		}

		final List<petrinetTrace.States.Value> values1 = getAllStateValues(state1);
		final List<petrinetTrace.States.Value> values2 = getAllStateValues(state2);
		...
		for (int i = 0; i < values1.size(); i++) {
			if (!respectIgnored || !isValueTraceIgnored(i)) {
				final petrinetTrace.States.Value value1 = values1.get(i);
				final petrinetTrace.States.Value value2 = values2.get(i);
				if (value1 != value2) {
	...
	public boolean compareSteps(EObject eObject1, EObject eObject2) {
		final petrinetTrace.Steps.SpecificStep step1;
		final petrinetTrace.Steps.SpecificStep step2;

		if (eObject1 instanceof petrinetTrace.Steps.SpecificStep) {
			step1 = (petrinetTrace.Steps.SpecificStep) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof petrinetTrace.Steps.SpecificStep) {
			step2 = (petrinetTrace.Steps.SpecificStep) eObject2;
		} else {
			return false;
		}
	...
	public boolean compareStatesWithSteps(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		...
		if (compareStates(state1, state2, respectIgnored)) {
			final List<petrinetTrace.Steps.SpecificStep> endedSteps1 = state1.getEndedSteps();
			final List<petrinetTrace.Steps.SpecificStep> startedSteps1 = state1.getStartedSteps();
			final List<petrinetTrace.Steps.SpecificStep> endedSteps2 = state2.getEndedSteps();
			final List<petrinetTrace.Steps.SpecificStep> startedSteps2 = state2.getStartedSteps();
			if (endedSteps1.size() == endedSteps2.size() && startedSteps1.size() == startedSteps2.size()) {
				boolean result = true;
				for (int i = 0; i < endedSteps1.size(); i++) {
					final petrinetTrace.Steps.SpecificStep endedStep1 = endedSteps1.get(i);
					final petrinetTrace.Steps.SpecificStep endedStep2 = endedSteps2.get(i);
				...
				for (int i = 0; i < startedSteps1.size(); i++) {
					final petrinetTrace.Steps.SpecificStep startedStep1 = startedSteps1.get(i);
					final petrinetTrace.Steps.SpecificStep startedStep2 = startedSteps2.get(i);
	...
	public boolean compareTraces(EObject eObject1, EObject eObject2, boolean respectIgnored) {
		final petrinetTrace.SpecificTrace trace1;
		final petrinetTrace.SpecificTrace trace2;

		if (eObject1 instanceof petrinetTrace.SpecificTrace) {
			trace1 = (petrinetTrace.SpecificTrace) eObject1;
		} else {
			return false;
		}

		if (eObject2 instanceof petrinetTrace.SpecificTrace) {
			trace2 = (petrinetTrace.SpecificTrace) eObject2;
		} else {
			return false;
		}

		final List<petrinetTrace.States.State> states1 = trace1.getStatesTrace();
		final List<petrinetTrace.States.State> states2 = trace2.getStatesTrace();
		...
		for (int i = 0; i < states1.size(); i++) {
			final petrinetTrace.States.State state1 = states1.get(i);
			final petrinetTrace.States.State state2 = states2.get(i);
	...
	private List<petrinetTrace.States.Value> getAllStateValues(petrinetTrace.States.State state) {
		return getAllStateValues(state, false);
	}

	private List<petrinetTrace.States.Value> getAllStateValues(petrinetTrace.States.State state,
			boolean includeHiddenValues) {
		final List<petrinetTrace.States.Value> result = new ArrayList<>();
		for (int i = 0; i < valueTraces.size(); i++) {
			if (includeHiddenValues || !isValueTraceIgnored(i)) {
				final List<? extends petrinetTrace.States.Value> trace = valueTraces.get(i);
				boolean notFound = true;
				for (petrinetTrace.States.Value value : trace) {
					if (value.getStatesNoOpposite().contains(state)) {
						result.add(value);
						notFound = false;
						break;
	...
	private boolean isStateBreakable(petrinetTrace.States.State state) {
	...
	public StateWrapper getStateWrapper(EObject state) {
		if (state instanceof petrinetTrace.States.State) {
			final int idx = statesTrace.indexOf(state);
			if (idx != -1) {
				final petrinetTrace.States.State state_cast = (petrinetTrace.States.State) state;
	...
	public void statesAdded(List<EObject> states) {
		updateEquivalenceClasses(states.stream().map(e -> (petrinetTrace.States.State) e).collect(Collectors.toList()));
		notifyListeners();
	}
